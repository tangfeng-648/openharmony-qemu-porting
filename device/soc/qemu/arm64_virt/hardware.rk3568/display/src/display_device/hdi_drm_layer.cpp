...
void DrmGemBuffer::Init(int drmFd, HdiLayerBuffer &hdl)
{
    ...
    offsets[0] = 0;
    DISPLAY_LOGI("w: %{public}d  h: %{public}d mDrmFormat : 0x%{public}x", hdl.GetWight(), hdl.GetHeight(), mDrmFormat);
//    ret = drmModeAddFB2(drmFd, hdl.GetWight(), hdl.GetHeight(), mDrmFormat, gemHandles, pitches, offsets, &mFdId, 0);
    ret = drmModeAddFB2(drmFd, hdl.GetWight(), hdl.GetHeight(), DRM_FORMAT_ARGB8888, gemHandles, pitches, offsets, &mFdId, 0);
