...
void DrmGemBuffer::Init(int drmFd, HdiLayerBuffer &hdl)
{
    ...
    offsets[0] = 0;
    DISPLAY_LOGI("w: %{public}d  h: %{public}d mDrmFormat : 0x%{public}x", hdl.GetWidth(), hdl.GetHeight(), mDrmFormat);
#ifdef PLANE_CURSOR_SUPPORT
    if(hdl.GetWidth() == DEFAULT_CURSOR_WIDTH && hdl.GetHeight() == DEFAULT_CURSOR_HEIGHT )
        ret = drmModeAddFB2(drmFd, hdl.GetWidth(), hdl.GetHeight(), DRM_FORMAT_XRGB8888, gemHandles, pitches, offsets, &mFdId, 0);
    else   
#endif    
        ret = drmModeAddFB2(drmFd, hdl.GetWidth(), hdl.GetHeight(), DRM_FORMAT_XRGB8888, gemHandles, pitches, offsets, &mFdId, 0);
    DISPLAY_LOGD("mGemHandle %{public}d  mFdId %{public}d", mGemHandle, mFdId);
    DISPLAY_LOGD("w: %{public}d  h: %{public}d mDrmFormat : %{public}x gemHandles: %{public}d pitches: %{public}d "
        "offsets: %{public}d",
...
void DrmGemBuffer::releaseBuffer()
{
...
//    return (mGemHandle != INVALID_DRM_ID) && (mFdId != INVALID_DRM_ID);
    return (mGemHandle != INVALID_DRM_ID) && (mFdId >= 0);
}
