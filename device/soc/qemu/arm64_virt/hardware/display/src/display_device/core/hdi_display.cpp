...
int32_t HdiDisplay::Init()
{
    ...
    auto cursorlayer = CreateHdiLayer(LAYER_TYPE_GRAPHIC);
    mCursorLayer = std::move(cursorlayer);
    #endif
...
int32_t HdiDisplay::PrepareDisplayLayers(bool *needFlushFb)
{
    ...
    for (auto &layer : layers) {
        ...
        if(layer->GetCompositionType() == COMPOSITION_CURSOR){
            ...
            mCursorLayer->SetLayerBuffer(&(layerBuffer->mHandle), -1);
            mCursorLayer->SetLayerRegion(&(layer->GetLayerRect()));
            mComposer->setCursorLayer(*mCursorLayer);
        }
        #endif
