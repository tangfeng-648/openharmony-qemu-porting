...
public:
    ...
    int32_t ApplyCursor(HdiDrmLayer &layer, HdiLayer &hlayer, DrmPlane &drmPlane, drmModeAtomicReqPtr pset);
    int32_t SetCrtcProperty(DrmPlane &drmPlane, drmModeAtomicReqPtr pset, int32_t bufferW, int32_t bufferH, int32_t x, int32_t y);
    int32_t SetCursorLayer(HdiLayer &cursorLayer);
    bool isAnimationEnd_ = false;
    bool CheckbootCompleted();
    #endif
...
private:
    ...
    #ifdef PLANE_CURSOR_SUPPORT
    std::vector<std::shared_ptr<DrmPlane>> mCursorPlanes;
    #endif   
};
} // OHOS
} // HDI
