...
extern int rk_bc_detect_notifier_register(struct notifier_block *nb, enum bc_port_type *type);
extern int rk_bc_detect_notifier_unregister(struct notifier_block *nb);
#else
static inline int dwc_otg_check_dpdm(bool wait)
{
    return USB_BC_TYPE_DISCNT;
}

static inline int rk_bc_detect_notifier_register(struct notifier_block *nb, enum bc_port_type *type)
{
    return -EINVAL;
}
