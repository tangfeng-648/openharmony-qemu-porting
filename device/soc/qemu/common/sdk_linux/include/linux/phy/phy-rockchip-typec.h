...
int tcphy_dp_set_phy_config(struct phy *phy, int link_rate, int lanes, u8 swing, u8 pre_emp);
...
#else
static inline int tcphy_dp_set_phy_config(struct phy *phy, int link_rate, int lanes, u8 swing, u8 pre_emp)
{
    return -ENODEV;
}
...
static inline int tcphy_dp_set_link_rate(struct phy *phy, int link_rate, bool ssc_on)
{
    return -ENODEV;
}
#endif
