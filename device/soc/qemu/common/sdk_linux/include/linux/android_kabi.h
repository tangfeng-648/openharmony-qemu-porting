...
#define __ANDROID_KABI_CHECK_SIZE_ALIGN(_orig, _new)                                                                   \
    union {                                                                                                            \
        _Static_assert(sizeof(struct { _new; }) <= sizeof(struct { _orig; }), __FILE__                                 \
                       ":" __stringify(__LINE__) ": " __stringify(_new) " is larger than " __stringify(_orig));        \
        _Static_assert(                                                                                                \
            __alignof__(struct { _new; }) <= __alignof__(struct { _orig; }), __FILE__                                  \
            ":" __stringify(__LINE__) ": " __stringify(_orig) " is not aligned the same as " __stringify(_new));       \
    }
