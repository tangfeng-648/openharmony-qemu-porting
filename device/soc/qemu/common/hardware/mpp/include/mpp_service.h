...
#define REQ_DATA_PTR(ptr) ((unsigned int)(ptr))
...
typedef struct MppServiceCmdCap_t {
    unsigned int support_cmd;
    unsigned int query_cmd;
    unsigned int init_cmd;
    unsigned int send_cmd;
    unsigned int poll_cmd;
...
void check_mpp_service_cap(unsigned int *codec_type, unsigned int *hw_ids, MppServiceCmdCap *cap);
