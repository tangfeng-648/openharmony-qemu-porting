...
static int bridge_stop(struct rkisp_bridge_device *dev)
{
    ...
    ret = wait_event_timeout(dev->done, !dev->en, msecs_to_jiffies(BRIDGE_V30_MSES));
    if (!ret) {
        v4l2_warn(&dev->sd, "%s timeout ret:%d\n", __func__, ret);
    }
    crop_off(dev);
    dev->stopping = false;
    dev->en = false;
...
static void bridge_update_mi(struct rkisp_bridge_device *br)
{
    ...
    v4l2_dbg(RKISP_MAX_SENSOR, rkisp_debug, &br->sd, "update pic(shd:0x%x base:0x%x)\n",
             rkisp_read(dev, br->cfg->reg.y0_base_shd, true), rkisp_read(dev, br->cfg->reg.y0_base, true));
}
...
static int bridge_frame_end(struct rkisp_bridge_device *dev, u32 state)
{
    ...
    if (hw->cur_buf && hw->nxt_buf) {
        if (ispdev->skip_frame > 0) {
            ...
            ns = 0;
            rkisp_dmarx_get_frame(ispdev, &hw->cur_buf->frame_id, NULL, &ns, true);
            if (!ns) {
                ns = ktime_get_ns();
            }
...
static int config_mp(struct rkisp_bridge_device *dev)
{
    ...
    rkisp_write(dev->ispdev, ISP_MPFBC_BASE, dev->work_mode & ISP_ISPP_422, false);
    rkisp_set_bits(dev->ispdev, CIF_MI_CTRL, MI_CTRL_MP_FMT_MASK,
                   MI_CTRL_MP_WRITE_YUV_SPLA | CIF_MI_CTRL_MP_ENABLE | CIF_MI_MP_AUTOUPDATE_ENABLE, false);
    dev->ispdev->irq_ends_mask |= ISP_FRAME_MP;
    return 0;
}
...
{
    rkisp_clear_bits(dev->ispdev, CIF_MI_CTRL, CIF_MI_CTRL_MP_ENABLE | CIF_MI_CTRL_RAW_ENABLE, false);
}
