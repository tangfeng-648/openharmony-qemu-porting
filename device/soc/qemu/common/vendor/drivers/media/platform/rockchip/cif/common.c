...
int rkcif_alloc_buffer(struct rkcif_device *dev, struct rkcif_dummy_buffer *buf)
{
    ...
    v4l2_dbg(1, rkcif_debug, &dev->v4l2_dev, "%s buf:0x%x~0x%x size:%d\n", __func__, (u32)buf->dma_addr,
...
void rkcif_free_buffer(struct rkcif_device *dev, struct rkcif_dummy_buffer *buf)
{
    const struct vb2_mem_ops *g_ops = dev->hw_dev->mem_ops;

    if (buf && buf->mem_priv) {
        v4l2_dbg(1, rkcif_debug, &dev->v4l2_dev, "%s buf:0x%x~0x%x\n", __func__, (u32)buf->dma_addr,
...
static int rkcif_alloc_page_dummy_buf(struct rkcif_device *dev, struct rkcif_dummy_buffer *buf)
{
    ...
    buf->mem_priv = sg;
    buf->pages = pages;
    v4l2_dbg(1, rkcif_debug, &dev->v4l2_dev, "%s buf:0x%x map cnt:%d size:%d\n", __func__, (u32)buf->dma_addr, ret,
             buf->size);
    return 0;
free_sg:
    kfree(sg);
...
int rkcif_alloc_common_dummy_buf(struct rkcif_device *dev, struct rkcif_dummy_buffer *buf)
{
    ...
    if (!ret) {
        v4l2_dbg(1, rkcif_debug, &dev->v4l2_dev, "%s buf:0x%x size:%d\n", __func__, (u32)buf->dma_addr, buf->size);
    }
end:
    if (ret < 0) {
