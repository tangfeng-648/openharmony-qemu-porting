...
static int rkcif_luma_enum_fmt_meta_cap(struct file *file, void *priv, struct v4l2_fmtdesc *f)
{
...
static int rkcif_luma_g_fmt_meta_cap(struct file *file, void *priv, struct v4l2_format *f)
{
    struct video_device *video = video_devdata(file);
    struct rkcif_luma_vdev *luma_vdev = video_get_drvdata(video);
...
static int rkcif_luma_querycap(struct file *file, void *priv, struct v4l2_capability *cap)
{
...
static const struct v4l2_ioctl_ops rkcif_luma_ioctl = {.vidioc_reqbufs = vb2_ioctl_reqbufs,
...
struct v4l2_file_operations rkcif_luma_fops = {.mmap = vb2_fop_mmap,
                                               .unlocked_ioctl = video_ioctl2,
                                               .poll = vb2_fop_poll,
                                               .open = rkcif_luma_fh_open,
                                               .release = rkcif_luma_fop_release};

static int rkcif_luma_vb2_queue_setup(struct vb2_queue *vq, unsigned int *num_buffers, unsigned int *num_planes,
                                      unsigned int sizes[], struct device *alloc_ctxs[])
{
    ...
    *num_buffers = clamp_t(u32, *num_buffers, RKCIF_LUMA_REQ_BUFS_MIN, RKCIF_LUMA_REQ_BUFS_MAX);
...
static void rkcif_luma_vb2_stop_streaming(struct vb2_queue *vq)
{
    ...
    for (i = 0; i < RKCIF_LUMA_REQ_BUFS_MAX; i++) {
        ...
        buf = list_first_entry(&luma_vdev->stat, struct rkcif_buffer, queue);
        list_del(&buf->queue);
...
static int rkcif_luma_vb2_start_streaming(struct vb2_queue *queue, unsigned int count)
{
...
static int rkcif_luma_init_vb2_queue(struct vb2_queue *q, struct rkcif_luma_vdev *luma_vdev)
{
...
static void rkcif_stats_send_luma(struct rkcif_luma_vdev *vdev, struct rkcif_luma_readout_work *work)
{
    ...
    if (!list_empty(&vdev->stat)) {
        cur_buf = list_first_entry(&vdev->stat, struct rkcif_buffer, queue);
...
static void rkcif_luma_readout_task(unsigned long data)
{
    ...
    struct rkcif_luma_vdev *vdev = (struct rkcif_luma_vdev *)data;
...
void rkcif_luma_isr(struct rkcif_luma_vdev *luma_vdev, int mipi_id, u32 frame_id)
{
    ...
    if (mipi_id == RKCIF_STREAM_MIPI_ID0 && !luma_vdev->ystat_rdflg[0]) {
        ...
        for (i = 0; i < ISP2X_MIPI_LUMA_MEAN_MAX; i++) {
            luma_vdev->work.luma[0].exp_mean[i] = rkcif_read_register(luma_vdev->cifdev, CIF_REG_Y_STAT_VALUE);
        }
    ...
    if (mipi_id == RKCIF_STREAM_MIPI_ID1 && !luma_vdev->ystat_rdflg[1]) {
        ...
        rkcif_write_register(luma_vdev->cifdev, CIF_REG_Y_STAT_CONTROL, value);
        for (i = 0; i < ISP2X_MIPI_LUMA_MEAN_MAX; i++) {
            luma_vdev->work.luma[1].exp_mean[i] = rkcif_read_register(luma_vdev->cifdev, CIF_REG_Y_STAT_VALUE);
        }
    ...
    if (mipi_id == RKCIF_STREAM_MIPI_ID2 && !luma_vdev->ystat_rdflg[2]) {
        ...
        rkcif_write_register(luma_vdev->cifdev, CIF_REG_Y_STAT_CONTROL, value);
        for (i = 0; i < ISP2X_MIPI_LUMA_MEAN_MAX; i++) {
            luma_vdev->work.luma[2].exp_mean[i] = rkcif_read_register(luma_vdev->cifdev, CIF_REG_Y_STAT_VALUE);
        }
...
void rkcif_start_luma(struct rkcif_luma_vdev *luma_vdev, const struct cif_input_fmt *cif_fmt_in)
{
    ...
    rkcif_write_register(luma_vdev->cifdev, CIF_REG_Y_STAT_CONTROL, SW_Y_STAT_BAYER_TYPE(bayer) | SW_Y_STAT_EN);
...
int rkcif_register_luma_vdev(struct rkcif_luma_vdev *luma_vdev, struct v4l2_device *v4l2_dev, struct rkcif_device *dev)
{
    ...
    ret = kfifo_alloc(&luma_vdev->rd_kfifo, RKCIF_LUMA_READOUT_WORK_SIZE, GFP_KERNEL);
    if (ret) {
        dev_err(&vdev->dev, "kfifo_alloc failed with error %d\n", ret);
        goto err_unregister_video;
    }

    tasklet_init(&luma_vdev->rd_tasklet, rkcif_luma_readout_task, (unsigned long)luma_vdev);
    tasklet_disable(&luma_vdev->rd_tasklet);
