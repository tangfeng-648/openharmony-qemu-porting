...
int kbase_debugfs_helper_set_attr_from_string(const char *const buf, void *const array, size_t const nelems,
                                              kbase_debugfs_helper_set_attr_fn const set_attr_fn)
{
...
ssize_t kbase_debugfs_helper_get_attr_to_string(char *const buf, size_t const size, void *const array,
                                                size_t const nelems, kbase_debugfs_helper_get_attr_fn const get_attr_fn)
{
    ssize_t total = 0;
    size_t index;
    ...
    for (index = 0; index < nelems; ++index) {
        ...
        total += scnprintf(buf + total, size - total, "%zu%s", get_attr_fn(array, index), postfix);
    }
