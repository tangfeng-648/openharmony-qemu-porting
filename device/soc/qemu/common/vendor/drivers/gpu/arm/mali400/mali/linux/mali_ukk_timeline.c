                                      ...
                                      mali_uk_timeline_get_latest_point_s __user *uargs)
{
    ...
    if (0 != get_user(val, &uargs->timeline)) {
        return -EFAULT;
    }
...
int timeline_wait_wrapper(struct mali_session_data *session, mali_uk_timeline_wait_s __user *uargs)
{
    ...
    if (0 != copy_from_user(&uk_fence, &uargs->fence, sizeof(mali_uk_fence_t))) {
        return -EFAULT;
    }
    if (0 != get_user(timeout, &uargs->timeout)) {
        return -EFAULT;
    }
                                       ...
                                       mali_uk_timeline_create_sync_fence_s __user *uargs)
{
    ...
    if (0 != put_user(sync_fd, &uargs->sync_fd)) {
        return -EFAULT;
    }
