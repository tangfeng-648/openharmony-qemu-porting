...
void kbase_backend_gpuprops_get(struct kbase_device *kbdev, struct kbase_gpuprops_regdump *regdump)
{
    ...
    regdump->suspend_size = kbase_reg_read(kbdev, GPU_CONTROL_REG(SUSPEND_SIZE), NULL);
    ...
    regdump->thread_max_threads = kbase_reg_read(kbdev, GPU_CONTROL_REG(THREAD_MAX_THREADS), NULL);
    regdump->thread_max_workgroup_size = kbase_reg_read(kbdev, GPU_CONTROL_REG(THREAD_MAX_WORKGROUP_SIZE), NULL);
    regdump->thread_max_barrier_size = kbase_reg_read(kbdev, GPU_CONTROL_REG(THREAD_MAX_BARRIER_SIZE), NULL);
    regdump->thread_features = kbase_reg_read(kbdev, GPU_CONTROL_REG(THREAD_FEATURES), NULL);
    ...
    regdump->stack_present_lo = kbase_reg_read(kbdev, GPU_CONTROL_REG(STACK_PRESENT_LO), NULL);
    regdump->stack_present_hi = kbase_reg_read(kbdev, GPU_CONTROL_REG(STACK_PRESENT_HI), NULL);
}
...
void kbase_backend_gpuprops_get_features(struct kbase_device *kbdev, struct kbase_gpuprops_regdump *regdump)
{
    if (kbase_hw_has_feature(kbdev, BASE_HW_FEATURE_COHERENCY_REG)) {
        ...
        regdump->coherency_features = kbase_reg_read(kbdev, GPU_CONTROL_REG(COHERENCY_FEATURES), NULL);
