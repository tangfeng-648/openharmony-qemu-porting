...
void kbase_mem_pool_config_debugfs_set_max_size(void *const array, size_t const index, size_t const value)
{
...
{
    CSTD_UNUSED(data);
    return kbase_debugfs_helper_seq_read(sfile, MEMORY_GROUP_MANAGER_NR_GROUPS, kbase_mem_pool_debugfs_size);
}

static ssize_t kbase_mem_pool_debugfs_write(struct file *file, const char __user *ubuf, size_t count, loff_t *ppos)
{
    int err;

    CSTD_UNUSED(ppos);
    err =
        kbase_debugfs_helper_seq_write(file, ubuf, count, MEMORY_GROUP_MANAGER_NR_GROUPS, kbase_mem_pool_debugfs_trim);
...
{
    CSTD_UNUSED(data);
    return kbase_debugfs_helper_seq_read(sfile, MEMORY_GROUP_MANAGER_NR_GROUPS, kbase_mem_pool_debugfs_max_size);
}

static ssize_t kbase_mem_pool_debugfs_max_size_write(struct file *file, const char __user *ubuf, size_t count,
                                                     loff_t *ppos)
{
    int err;
...
void kbase_mem_pool_debugfs_init(struct dentry *parent, struct kbase_context *kctx)
{
    ...
    debugfs_create_file("lp_mem_pool_size", mode, parent, &kctx->mem_pools.large, &kbase_mem_pool_debugfs_fops);
