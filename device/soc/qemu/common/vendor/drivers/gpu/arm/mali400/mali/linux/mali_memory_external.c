...
mali_osk_errcode_t mali_mem_bind_ext_buf(mali_mem_allocation *alloc, mali_mem_backend *mem_backend, u32 phys_addr,
                                         u32 flag)
{
    struct mali_session_data *session;
    ...
    if (size % MALI_OSK_MALI_PAGE_SIZE) {
        MALI_ERROR(MALI_OSK_ERR_INVALID_ARGS);
    }
    ...
    if (alloc->flags & MALI_MEM_FLAG_MALI_GUARD_PAGE) {
        mali_mmu_pagedir_update(session->page_directory, virt + size, phys, MALI_OSK_MALI_PAGE_SIZE,
                                MALI_MMU_FLAGS_DEFAULT);
    }
    MALI_DEBUG_PRINT(MALI_KERNEL_LEVEL_MESSAGE,
                     ("Requested to map physical memory 0x%x-0x%x into virtual memory 0x%x\n", phys_addr,
                      (phys_addr + size - 1), virt));
    mali_session_memory_unlock(session);
