...
void kbase_mmu_get_as_setup(struct kbase_mmu_table *mmut, struct kbase_mmu_setup *const setup)
{
    ...
    setup->memattr =
        ...
        (AS_MEMATTR_FORCE_TO_CACHE_ALL << (AS_MEMATTR_INDEX_FORCE_TO_CACHE_ALL * GPU_FAULTSTATUS_ACCESS_TYPE_SHIFT)) |
        (AS_MEMATTR_WRITE_ALLOC << (AS_MEMATTR_INDEX_WRITE_ALLOC * GPU_FAULTSTATUS_ACCESS_TYPE_SHIFT)) |
        (AS_MEMATTR_AARCH64_OUTER_IMPL_DEF << (AS_MEMATTR_INDEX_OUTER_IMPL_DEF * GPU_FAULTSTATUS_ACCESS_TYPE_SHIFT)) |
        (AS_MEMATTR_AARCH64_OUTER_WA << (AS_MEMATTR_INDEX_OUTER_WA * GPU_FAULTSTATUS_ACCESS_TYPE_SHIFT)) |
        (AS_MEMATTR_AARCH64_NON_CACHEABLE << (AS_MEMATTR_INDEX_NON_CACHEABLE * GPU_FAULTSTATUS_ACCESS_TYPE_SHIFT));
...
void kbase_gpu_report_bus_fault_and_kill(struct kbase_context *kctx, struct kbase_as *as, struct kbase_fault *fault)
{
    ...
    dev_err(kbdev->dev,
            ...
            "exception data 0x%X\n"
            "pid: %d\n",
            as_no, fault->addr, status, exception_type, kbase_gpu_exception_name(exception_type), exception_data,
            kctx->pid);
                                     ...
                                     struct kbase_fault *fault)
{
    ...
    dev_err(kbdev->dev,
            ...
            "source id 0x%X\n"
            "pid: %d\n",
            as_no, fault->addr, reason_str, fault->status, exception_type, kbase_gpu_exception_name(exception_type),
            access_type, kbase_gpu_access_type_name(fault->status), source_id, kctx->pid);

    /* hardware counters dump fault handling */
    if ((kbdev->hwcnt.kctx) && (kbdev->hwcnt.kctx->as_nr == as_no) &&
        (kbdev->hwcnt.backend.state == KBASE_INSTR_STATE_DUMPING)) {
        if ((fault->addr >= kbdev->hwcnt.addr) && (fault->addr < (kbdev->hwcnt.addr + kbdev->hwcnt.addr_bytes))) {
 ...
 */
static void kbase_mmu_interrupt_process(struct kbase_device *kbdev, struct kbase_context *kctx, struct kbase_as *as,
                                        struct kbase_fault *fault)
{
    lockdep_assert_held(&kbdev->hwaccess_lock);

    dev_dbg(kbdev->dev, "Entering %s kctx %p, as %p\n", __func__, (void *)kctx, (void *)as);

    if (!kctx) {
        dev_warn(kbdev->dev, "%s in AS%d at 0x%016llx with no context present! Spurious IRQ or SW Design Error?\n",
                 kbase_as_has_bus_fault(as, fault) ? "Bus error" : "Page fault", as->number, fault->addr);
    ...
    dev_dbg(kbdev->dev, "Leaving %s kctx %p, as %p\n", __func__, (void *)kctx, (void *)as);
}
...
static void validate_protected_page_fault(struct kbase_device *kbdev)
{
    ...
    if (kbase_hw_has_feature(kbdev, BASE_HW_FEATURE_PROTECTED_DEBUG_MODE)) {
        protected_debug_mode = kbase_reg_read(kbdev, GPU_CONTROL_REG(GPU_STATUS)) & GPU_DBGEN;
    }
...
void kbase_mmu_interrupt(struct kbase_device *kbdev, u32 irq_stat)
{
    ...
    while (bf_bits | pf_bits) {
        ...
        fault->addr = kbase_reg_read(kbdev, MMU_AS_REG(as_no, AS_FAULTADDRESS_HI));
        fault->addr <<= AS_FAULTADDRESS_LO;
        fault->addr |= kbase_reg_read(kbdev, MMU_AS_REG(as_no, AS_FAULTADDRESS_LO));
        ...
        fault->status = kbase_reg_read(kbdev, MMU_AS_REG(as_no, AS_FAULTSTATUS));

        if (kbase_hw_has_feature(kbdev, BASE_HW_FEATURE_AARCH64_MMU)) {
            fault->extra_addr = kbase_reg_read(kbdev, MMU_AS_REG(as_no, AS_FAULTEXTRA_HI));
            fault->extra_addr <<= AS_FAULTADDRESS_LO;
            fault->extra_addr |= kbase_reg_read(kbdev, MMU_AS_REG(as_no, AS_FAULTEXTRA_LO));
        }
...
{
    dev_dbg(kctx->kbdev->dev, "Switching to incremental rendering for region %p\n", (void *)reg);
