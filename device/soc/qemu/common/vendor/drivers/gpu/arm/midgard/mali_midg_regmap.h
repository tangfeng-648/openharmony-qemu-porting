...
#ifndef MIDGARD_REGMAP_H_
#define MIDGARD_REGMAP_H_
...
#define SUSPEND_SIZE                                                                                                   \
    0x008                     /* (RO) Fixed-function suspend buffer                                                    \
              size */
...
#define RESET_COMPLETED                                                                                                \
    (1 << 8)                          /* Set when a reset has completed. Intended to use with SOFT_RESET               \
                     commands which may take time. */
#define POWER_CHANGED_SINGLE (1 << 9) /* Set when a single core has finished powering up or down. */
#define POWER_CHANGED_ALL                                                                                              \
    (1 << 10) /* Set when all cores have finished powering up or down                                                  \
and the power manager is idle. */

#define PRFCNT_SAMPLE_COMPLETED (1 << 16) /* Set when a performance count sample has completed. */
#define CLEAN_CACHES_COMPLETED (1 << 17)  /* Set when a cache clean operation has completed. */

#define GPU_IRQ_REG_ALL                                                                                                \
    (GPU_FAULT | MULTIPLE_GPU_FAULTS | RESET_COMPLETED | POWER_CHANGED_ALL | PRFCNT_SAMPLE_COMPLETED)
...
#define JOB_IRQ_THROTTLE                                                                                               \
    0x014 /* cycles to delay delivering an interrupt externally. The JOB_IRQ_STATUS is NOT affected by this, just the  \
...
#define JS_XAFFINITY                                                                                                   \
    0x1C /* (RO) Extended affinity mask for job                                                                        \
slot n */
...
#define JS_XAFFINITY_NEXT                                                                                              \
    0x5C /* (RW) Next extended affinity mask for                                                                       \
job slot n */
...
#define AS_COMMAND_FLUSH                                                                                               \
    0x04                         /* Flush all L2 caches then issue a flush region command to all MMUs                  \
                 (deprecated - only for use with T60x) */
#define AS_COMMAND_FLUSH_PT 0x04 /* Flush all L2 caches then issue a flush region command to all MMUs */
#define AS_COMMAND_FLUSH_MEM                                                                                           \
    0x05 /* Wait for memory accesses to complete, flush all the L1s cache then                                         \
...
#define PRFCNT_CONFIG_MODE_MANUAL                                                                                      \
    1 /* The performance counters are enabled, but are only written out when a PRFCNT_SAMPLE command is issued using   \
         the GPU_COMMAND register. */
#define PRFCNT_CONFIG_MODE_TILE                                                                                        \
    2 /* The performance counters are enabled, and are written out each time a tile finishes rendering. */
