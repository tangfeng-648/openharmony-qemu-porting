...
#define DBG_MSG(level, args)                                                                                           \
    do { /* args should be in brackets */                                                                              \
        ((level) <= ump_debug_level) ? UMP_DEBUG_PRINT(("UMP<" #level ">: ")), UMP_DEBUG_PRINT(args) : 0;              \
    } while (0)
...
#define DEBUG_ASSERT_POINTER(pointer)                                                                                  \
    do {                                                                                                               \
        if ((pointer) == NULL)                                                                                         \
            MSG_ERR(("NULL pointer " #pointer));                                                                       \
    } while (0)
#define DEBUG_ASSERT(condition)                                                                                        \
    do {                                                                                                               \
        if (!(condition))                                                                                              \
            MSG_ERR(("ASSERT failed: " #condition));                                                                   \
    } while (0)
#else /* DEBUG */
...
typedef struct ump_session_data {
    _mali_osk_list_t list_head_session_memory_list; /**< List of ump allocations made by the process (elements are
