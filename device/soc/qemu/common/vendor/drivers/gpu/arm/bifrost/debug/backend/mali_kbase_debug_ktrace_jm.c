...
{
    *written += MAX(snprintf(buffer + *written, MAX(sz - *written, 0), "katom,gpu_addr,jobslot,refcount"), 0);
}
...
void kbasep_ktrace_backend_format_msg(struct kbase_ktrace_msg *trace_msg, char *buffer, int sz, s32 *written)
{
    ...
    if (trace_msg->backend.flags & KBASE_KTRACE_FLAG_JM_ATOM) {
        *written += MAX(snprintf(buffer + *written, MAX(sz - *written, 0), "atom %d (ud: 0x%llx 0x%llx)",
                                 trace_msg->backend.atom_number, trace_msg->backend.atom_udata[0],
                                 trace_msg->backend.atom_udata[1]),
                        0);
    }

    /* gpu_addr */
    if (trace_msg->backend.flags & KBASE_KTRACE_FLAG_BACKEND) {
        *written += MAX(snprintf(buffer + *written, MAX(sz - *written, 0), ",%.8llx,", trace_msg->backend.gpu_addr), 0);
    } else {
    ...
    if (trace_msg->backend.flags & KBASE_KTRACE_FLAG_JM_JOBSLOT) {
        *written += MAX(snprintf(buffer + *written, MAX(sz - *written, 0), "%d", trace_msg->backend.jobslot), 0);
    }
    ...
    *written += MAX(snprintf(buffer + *written, MAX(sz - *written, 0), ","), 0);
    ...
    if (trace_msg->backend.flags & KBASE_KTRACE_FLAG_JM_REFCOUNT) {
        *written += MAX(snprintf(buffer + *written, MAX(sz - *written, 0), "%d", trace_msg->backend.refcount), 0);
    }
}
