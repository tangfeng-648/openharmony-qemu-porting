...
#define MALI_MMU_FLAGS_FORCE_GP_READ_ALLOCATE                                                                          \
    (MALI_MMU_FLAGS_PRESENT | MALI_MMU_FLAGS_READ_PERMISSION | MALI_MMU_FLAGS_WRITE_PERMISSION |                       \
     MALI_MMU_FLAGS_OVERRIDE_CACHE | MALI_MMU_FLAGS_WRITE_CACHEABLE | MALI_MMU_FLAGS_WRITE_BUFFERABLE |                \
     MALI_MMU_FLAGS_READ_CACHEABLE | MALI_MMU_FLAGS_READ_ALLOCATE)
...
struct mali_page_directory {
    ...
    mali_io_address page_directory_mapped; /**< Pointer to the mapped version of the page directory into the kernel's
                                              address space */

    mali_io_address page_entries_mapped[1024]; /**< Pointers to the page tables which exists in the page directory
                                                  mapped into the kernel's address space */
    u32 page_entries_usage_count[1024]; /**< Tracks usage count of the page table pages, so they can be releases on the
                                           last reference */
};
