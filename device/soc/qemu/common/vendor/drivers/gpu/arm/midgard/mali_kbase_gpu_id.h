...
#ifndef KBASE_GPU_ID_H_
#define KBASE_GPU_ID_H_
...
#define GPU_ID_IS_NEW_FORMAT(product_id) ((product_id) != GPU_ID_PI_T60X && (product_id) >= GPU_ID_PI_NEW_FORMAT_START)
...
#define GPU_ID2_PRODUCT_MAKE(arch_major, arch_minor, arch_rev, product_major)                                          \
    (((arch_major) << GPU_ID2_ARCH_MAJOR_SHIFT) | ((arch_minor) << GPU_ID2_ARCH_MINOR_SHIFT) |                         \
     ((arch_rev) << GPU_ID2_ARCH_REV_SHIFT) | ((product_major) << GPU_ID2_PRODUCT_MAJOR_SHIFT))
...
#define GPU_ID2_VERSION_MAKE(version_major, version_minor, version_status)                                             \
    (((version_major) << GPU_ID2_VERSION_MAJOR_SHIFT) | ((version_minor) << GPU_ID2_VERSION_MINOR_SHIFT) |             \
     ((version_status) << GPU_ID2_VERSION_STATUS_SHIFT))

/* Helper macro to create a complete GPU_ID (new format) */
#define GPU_ID2_MAKE(arch_major, arch_minor, arch_rev, product_major, version_major, version_minor, version_status)    \
...
#define GPU_ID2_MODEL_MAKE(arch_major, product_major)                                                                  \
    (((arch_major) << GPU_ID2_ARCH_MAJOR_SHIFT) | ((product_major) << GPU_ID2_PRODUCT_MAJOR_SHIFT))
...
/* Strip off the non-relevant bits from a product_id value and make it suitable
   ...
   model. */
#define GPU_ID2_MODEL_MATCH_VALUE(product_id) (((product_id) << GPU_ID2_PRODUCT_MAJOR_SHIFT) & GPU_ID2_PRODUCT_MODEL)
...
#define GPU_ID_MAKE(id, major, minor, status)                                                                          \
    (((id) << GPU_ID_VERSION_PRODUCT_ID_SHIFT) | ((major) << GPU_ID_VERSION_MAJOR_SHIFT) |                             \
     ((minor) << GPU_ID_VERSION_MINOR_SHIFT) | ((status) << GPU_ID_VERSION_STATUS_SHIFT))
